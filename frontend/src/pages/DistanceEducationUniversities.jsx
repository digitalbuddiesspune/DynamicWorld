...
export default function DistanceEducationUniversities() {
  const API = import.meta.env.VITE_BACKEND_API; // e.g. https://api.dynamicworld.in
  ...
  const fetchList = useCallback(async () => {
    try {
      ...
      let res = await fetch(`${API}/getAllUniversitiesV2`);
      if (!res.ok) {
        // try fallbacks if the first fails
        const alt = await fetch(`${API}/getAllUniversitiesV2`);
