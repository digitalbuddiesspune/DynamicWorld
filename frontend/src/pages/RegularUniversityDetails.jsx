...
const CourseRow = ({ c }) => {
  ...
  return (
    <tr className="odd:bg-white even:bg-gray-50 hover:bg-[#3E96F4]/5">
...
export default function RegularUniversityDetails() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const { data } = await axios.get(`${API}/regularUniversity/${slug}`, { signal: controller.signal });
      // Backend shape: { success, data, message } — be tolerant
  ...
  const title = useMemo(() => firstNonEmpty(name, slug), [name, slug]);
    ...
    <div className="text-[#31393C]">
      ...
      <main className="mx-auto max-w-7xl px-2 lg:px-0">
        <div className="py-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8">
            ...
            <div className="flex flex-col lg:flex-row sm:items-center gap-4 sm:gap-6">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-sm sm:text-base text-gray-600">
                  {firstNonEmpty(location)} <span className="mx-2">•</span> Established: {firstNonEmpty(established)}
                </p>
          ...
          {(description || primary_focus || accreditation.length > 0 || awards.length > 0 || website) && (
            <Section title="About the University">
              {description && <p className="text-sm sm:text-base text-gray-700">{description}</p>}

              {primary_focus && (
                <div className="mt-4">
                  <h3 className="my-3 text-lg sm:text-xl font-semibold text-gray-800">Primary Focus</h3>
                  <p className="text-sm sm:text-base text-gray-700">{primary_focus}</p>
                </div>
              )}
                ...
                <div className="mt-4">
                  ...
                  <ul className="list-disc list-inside space-y-2 text-sm sm:text-base text-gray-700">
                    {accreditation.map((item, i) => (
                      <li key={`acc-${i}`} className="leading-relaxed">{item}</li>
                    ))}
                  </ul>
                </div>
              )}
              ...
              {!!awards.length && (
                <div className="mt-4">
                  <h3 className="my-3 text-lg sm:text-xl font-semibold text-gray-800">Awards / Rankings</h3>
                  <ul className="list-disc list-inside space-y-2 text-sm sm:text-base text-gray-700">
                    {awards.map((item, i) => (
                      <li key={`awd-${i}`} className="leading-relaxed">{item}</li>
                    ))}
                  </ul>
                </div>
              )}
              ...
              {naacBadgeUrl && (
                <div className="mt-4">
                  <img src={naacBadgeUrl} alt={`NAAC ${naacGrade} Badge`} className="h-12 sm:h-16 object-contain" loading="lazy" />
                </div>
              )}
          ...
          {sortedCourses.length > 0 && (
            <Section title="Programs" right={<Badge>{sortedCourses.length} listed</Badge>}>
                ...
                <table className="min-w-full text-left text-sm sm:text-base">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {sortedCourses.map((c, idx) => (
                      <CourseRow key={c?.id || `${c?.courseName}-${c?.specialization}-${idx}`} c={c} />
                    ))}
                  </tbody>
                </table>
              </div>
            ...
            <>
              {universityRanking
                ...
                .map((item, index) => {
                  ...
                  return (
                    <div key={`naac-${index}`} className="rounded-xl bg-white shadow-sm ring-1 ring-gray-200 p-4 sm:p-6">
                      ...
                      {img ? (
                        <img src={img} alt="NAAC Accreditation Badge" className="h-12 sm:h-16 object-contain mt-3" loading="lazy" />
                      ) : null}
                    </div>
                  );
                })}
              ...
              <Section title="University Rankings">
                <ol className="list-disc list-inside space-y-2 text-sm sm:text-base text-gray-700">
                  {universityRanking.map((rank, index) => (
                    <li key={`rank-${index}`} className="leading-relaxed">{rank}</li>
                  ))}
                </ol>
              </Section>
            </>
          ...
          {imageArray.length > 1 && (
            <Section title="Gallery" right={<Badge>{imageArray.length}</Badge>}>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                {imageArray.slice(0, 12).map((src, i) => (
